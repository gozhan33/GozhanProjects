...
namespace DemoApp
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            {
                Console.WriteLine("Using OpenAI API Key: " + openAiKey.Substring(0, 4) + "****");
            }
            var mitreProvider = new MitreAttackDataProvider();
            var agent = new OpenAIAgent.OpenAIAgent(mitreProvider, openAiKey);

            Console.WriteLine("Enter your prompt related to MITRE attack techniques.\n Default is 'Tell me the recent MITRE attack techniques related to AI':");
            var prompt = Console.ReadLine();
            if (string.IsNullOrWhiteSpace(prompt))
                prompt = "Tell me the recent MITRE attack techniques related to AI";

            Console.WriteLine("\nQuerying OpenAI agent for prompt: \n" + prompt + "\n");
            Console.WriteLine("Please wait, this may take a few seconds...\n");

            // Query the OpenAI agent with the provided prompt
